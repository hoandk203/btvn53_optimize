            ...
            <TableContainer sx={{ maxWidth: maxWidth, margin: "0 auto" }} component={Paper}>
                <Table aria-label="simple table">
                    <TableHead>
                        <TableRow>
                            {columns.map((column) => (
                                <TableCell className={style["text--red"]} width={column?.width} key={column.name}>
                                    {column.text}
                                </TableCell>
                            ))}
                        </TableRow>
                                        ...
                                        if (column.name === "action") {
                                            return (
                                                <TableCell key={`${ridx}${column.name}`}>
                                                    <EditIcon onClick={() => onUpdate(row)} sx={{ color: "green" }} className={"ma-2 pointer"} />
                                                    <DeleteOutlineIcon onClick={() => onDelete(row.id)} sx={{ color: "red" }} className={"ma-2 pointer"} />
                                                </TableCell>
                                            );
                                        }
                                        if (column.name === "img") {
